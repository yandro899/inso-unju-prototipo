...
def CrearPedido():
    ...
    layout_01 = [
        [sg.Text("Datos del solicitante*:"), sg.Input(key="in_username", disabled=True, default_text="{} {}".format(C.este_usuario["dni"], C.este_usuario["name"]))],
...
def VerPedidos():
    ...
    layout = [
        ...
        [
            sg.Button("Nuevo", key="btn_nuevopedido", visible=(C.EsAdministrativo() and C.EsDeContabilidad())), 
...
def RevisarPedido(cod):
    ...
    if C.EsDeContabilidad() or C.EsSecretario():
        if C.EsSecretario() and pedidojson["estado"] == "4":
            buttons = [sg.Button("Aprobar", k="btn_aprobar"), sg.Button("Anular", k="btn_anular")]
        elif C.EsAdministrativo() and pedidojson["estado"] in ["0", "1"]:
            buttons = [sg.Button("Cancelar", k="btn_cancelar")]
        elif C.EsDirectivo() and pedidojson["estado"] == "1":
